...
const Nodes: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 先获取工作流详情
    WorkflowTypeDetail(parseInt(id)).then((result) => {
  ...
  return (
    <PageContainer title={`工作流[${workflowType?.name ?? ''}]节点管理`}>
      <ProTable<WorkflowAPI.WorkflowNode, WorkflowAPI.PageParams>
        rowKey="id"
        search={false}
      ...
      />
      <ModalForm
        title={(workflowStepId > 0 ? '编辑' : '新增') + '节点'}
        ...
        onFinish={async (formData: any) => {
          if (!workflowType?.id) {
            message.success('当前节点不属于任何工作流，请确认');
            return false;
          }

          formData.type_id = workflowType.id;
          // 需要将节点序号转换为int
